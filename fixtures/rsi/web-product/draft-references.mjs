...
export async function verifyDraftReferences(browser, root) {
  ...
  try {
    ...
    const result = await page.evaluate(async () => {
      ...
      const reference = {snapshot:{sha256:'b'.repeat(64),byte_len:900},metadata:{source:{kind:'native',binding:{session_id:'source',header_sha256:'c'.repeat(64)}},target:{session_id:'target',header_sha256:'d'.repeat(64)},capture:{kind:'suffix',interval:{through_seq:'9007199254740993',fact_prefix_sha256:'e'.repeat(64),scanned_after_seq:'9007199254740992',retained_after_seq:'9007199254740992',retained_through_seq:'9007199254740993',scanned_bytes:400,omissions:['fact_limit']}},text_bytes:12},preview:'你好世界'};
