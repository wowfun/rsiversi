  ...
  async setup(count,run) {
    ...
    if(!input.dataset.perf) {
      ...
      input.addEventListener('input',()=>{
        const started=fixtureInputStarted,count=fixtureCount,run=fixtureRun;
        ...
        render({...fixtureView});
        requestAnimationFrame(()=>requestAnimationFrame(()=>fixtureSamples.push({blocks:count,run,input_to_paint_ms:performance.now()-started})));
      });
    }
    await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));
    return {blocks:fixturePane().transcript.querySelectorAll('.message').length,input:input.getBoundingClientRect().width,body:fixturePane().transcript.querySelector('.message-text')?.textContent};
  },
  async trajectory() {
    const data=fixtureKey==='main'?fixtureView.surfaces.main:fixtureView.panes[0];
