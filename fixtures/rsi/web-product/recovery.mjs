...
try {
  ...
  await context.addInitScript(() => {
    ...
    window.savedRecord = async session => {
      const db = await new Promise((resolve,reject) => { const request=indexedDB.open("rsi.composer",4); request.onsuccess=()=>resolve(request.result); request.onerror=()=>reject(request.error); });
