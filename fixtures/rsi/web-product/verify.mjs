...
import { verifyRetrieval } from "./retrieval.mjs";
import { verifyPlugins } from "./plugins.mjs";
import { verifySettingsForm } from "./settings-form.mjs";
...
import { verifyDraftReferences } from "./draft-references.mjs";
import { verifyDom } from "./dom.mjs";
import { verifyFiles } from "./files.mjs";
import { verifyCompletionPointer, verifyFilePicker, verifyReferenceDraft, verifyInputDialogRetirement } from "./input-resources.mjs";
...
try {
  for (const [name, engine] of [["chromium", chromium], ["firefox", firefox]]) {
    ...
    try {
      ...
      await writeFile(join(report, `${name}-draft-references.json`), JSON.stringify(await verifyDraftReferences(browser, root)));
      await verifyDom(browser, root, report, name);
      await verifySettingsForm(browser, root);
      ...
      await verifyFilePicker(page, left, service, report, name);
      await verifyCompletionPointer(page, left, service, report, name);
      await verifyPlugins(page, left, service, receipt, report, name);
      await verifyMcp(page, left, report, name);
      await verifyRetrieval(page, left, service, report, name);
      ...
      const approvalSetting = page.getByRole("checkbox", {name:"Settings / require_approval",exact:true});
      await approvalSetting.check();
      const identifier=page.getByRole('textbox',{name:'Settings / settings_id',exact:true});
      const previousId=await identifier.inputValue();await identifier.fill('');
      await page.getByRole('button',{name:'Save settings',exact:true}).click();
      await page.locator('.settings-field-error').filter({hasText:/invalid/i}).waitFor();
      assert(await approvalSetting.isChecked(),'validation failure preserves unsaved values');
      await identifier.fill(previousId);
      const sandbox=page.getByRole('combobox',{name:'Settings / sandbox',exact:true});
      const sandboxMode=await sandbox.inputValue();await sandbox.selectOption(sandboxMode);
      const numeric=page.getByRole('spinbutton',{name:'Settings / turn_budget / maximum_tool_calls',exact:true});
      await page.getByRole('checkbox',{name:'Include Settings / turn_budget / maximum_tool_calls',exact:true}).check();await numeric.fill('123');
      await page.screenshot({path:join(report,`${name}-settings-typed.png`)});
      await page.setViewportSize({width:420,height:860});await page.screenshot({path:join(report,`${name}-settings-typed-narrow.png`)});await page.setViewportSize({width:1440,height:980});
      const savingEditor = await approvalSetting.elementHandle();
      await page.getByRole("button", { name: "Save settings", exact: true }).click();
      ...
      await numeric.fill('18446744073709551615');
      await page.getByRole('button', {name:'Save settings',exact:true}).click();
      await page.locator('.settings-field-error').filter({hasText:'exact integer'}).waitFor();
      await page.getByRole('button', {name:'Edit as JSON',exact:true}).click();
      const exactEditor=page.getByRole('textbox', {name:'Settings JSON',exact:true});
      assert.equal(await page.locator('.settings-field-error').innerText(),'');
      const exactText=await exactEditor.inputValue();
      assert.match(exactText,/"maximum_tool_calls":18446744073709551615/);
      await page.screenshot({path:join(report,`${name}-settings-exact-json.png`)});
      await exactEditor.fill(exactText.replace('"maximum_tool_calls":18446744073709551615','"maximum_tool_calls":123'));
      const savingExact=await exactEditor.elementHandle();
      await page.getByRole('button', {name:'Save settings',exact:true}).click();
      ...
      const preferenceEditor = page.getByRole("checkbox", { name: "Settings / web / enter_submit",exact:true });
      await preferenceEditor.check();
...
for (const name of ["chromium", "firefox"]) {
  if (process.env.RSI_WEB_BROWSER && process.env.RSI_WEB_BROWSER !== name) continue;
  for (const probe of ["renderers", "rust-renderer", "recovery", "tasks", "preview-verify", "plan-review"]) {
