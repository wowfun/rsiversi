...
try {
  ...
  for(const name of ['skill_read','mcp_resource_read','web_fetch','web_search','present'])assert(called(first,name).length>0,`actual durable ToolIntent missing: ${name}`);
  const echoes=first.filter(fact=>fact.type==='tool_intent' && fact.name.startsWith('mcp__') && fact.arguments.message==='LIVE_MCP_ECHO');
  assert.equal(echoes.length,1,'expected one advertised MCP echo call');
  const echo=called(first,echoes[0].name)[0];assert(echo.result && !echo.result.result.is_error && JSON.stringify(echo.result).includes('LIVE_MCP_ECHO'));
