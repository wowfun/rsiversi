...
export async function verifyWorkerLifecycle(browser, root) {
  ...
  const wasm = `
    ...
    export async function command() { rejectCommit(new Error("fixture commit failed")); }
    export async function disconnect() { rejectCommit?.(new Error("retired")); return resource_snapshot(); }
  ...
  try {
    ...
    const results = await page.evaluate(async () => {
      const results = [];
      for (const mode of ["awaiting_ack", "committing", "connected_failure"]) {
        const worker = new Worker("/worker.js", { type: "module" });
        const events = [], pending = new Map();
        let id = 0, receiveFrame, receiveCommit, receiveFailure;
