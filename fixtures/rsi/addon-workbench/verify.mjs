...
for (const [name, engine] of [["chromium", chromium], ["firefox", firefox]]) {
  ...
  try {
    ...
    await pane.getByRole("textbox", { name: "Left message" }).fill("/plan on");
    ...
    await pane.getByRole("textbox", { name: "Left message" }).fill("Inspect independent addon");
    await pane.getByRole("button", { name: "Send ↗" }).click();
    ...
    await pane.getByRole("textbox", { name: "Left message" }).fill("/plan off");
    await pane.getByRole("button", { name: "Send ↗" }).click();
    await plan.filter({ hasText: '"enabled": false' }).waitFor();
    ...
    await page.waitForFunction(previous => !previous.isConnected, oldEditor); await oldEditor.dispose();
