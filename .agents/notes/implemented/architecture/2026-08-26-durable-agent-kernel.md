---
name: Durable Agent kernel over ordinary plugins
...
`AgentKernel` owns session headers, Fact sequencing, sequential turn
...
Session service because reconnectable idempotency requires an unambiguous
...
Store append batches share immutable Fact allocations so retries and atomic
in-memory staging do not duplicate large payloads. Observation handles retain
their byte reservation until their final clone drops, including after leaving
the stream for a renderer or transport. A separate read budget cannot account
for that lifetime. Immediate Capacity terminates an attempt without advancing
a delivered cursor; callers resume from the last delivered control/Fact pair.
Only one control or Fact page is retained at a time, and pages alternate while
both streams have data. Scoped revision watches carry no payload and keep
entries only for active subscribers; tree membership has its own watch because
broker interaction snapshots need a different invalidation boundary.

SQLite cold validation uses a dedicated read-only connection and one admission
