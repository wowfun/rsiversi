...
- `rsi-ai-protocol` owns provider-neutral semantic and durable wire contracts.
  Keep provider JSON, HTTP, and SSE syntax in concrete provider crates.
- Keep the caller interface deep: callers use typed Language or Image Local
  services; provider authors implement capability-specific adapter interfaces;
  transport adapters remain internal.
