...
use rsi_agent_session_protocol::{
    FrozenAgentProfile, SessionFactBody, SessionHeader, SessionId, TurnOutcome,
};
use rsi_agent_store_protocol::SessionStore;
use rsi_agent_testkit::{MemoryStore, MemoryStoreFactory};
use rsi_agent_turn_protocol::{
    FinalizationResult, SubmitImage, SubmitSession, SubmitTurn, SubmittedTurn,
    TurnFinalizationContract, TurnFinalizer, TurnServiceContract,
};
use rsi_ai_protocol::{
...
use rsi_tools_protocol::{
    Result as ToolResultType, RetainedToolResult, ToolDefinition, ToolError, ToolExecution,
    ToolExecutor, ToolRegistration, ToolResult, ToolRuntimeContract,
};
...
impl BaseStack {
    ...
    async fn submit_and_wait_with_sandbox(
        ...
        let submitted = turns
            .submit(SubmitTurn {
                session: SubmitSession::Fresh(header()),
                text: text.into(),
                model: None,
                sandbox,
            })
...
impl TurnFinalizer for HangingFinalizer {
    async fn finalize(
        ...
        _turn_id: &rsi_agent_session_protocol::TurnId,
    ) -> FinalizationResult<()> {
