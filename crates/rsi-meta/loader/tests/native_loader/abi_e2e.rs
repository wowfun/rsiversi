...
impl PluginFactory for DualCaptureFactory {
    fn identity(&self) -> FactoryIdentity {
        FactoryIdentity::builtin("dual-capability-client", "1")
    }
...
#[tokio::test]
async fn real_dynamic_library_runs_through_loader_service_and_dynamically_provides_echo() {
    ...
    let loader_service = apply_loader_service(&runtime, catalog.clone(), "loader-client").await;
    let command = json!({
        "operation": "load",
        "id": "native-echo",
    ...
    let response = loader_service
        .invoke(Message::new(serde_json::to_vec(&command).unwrap()))
        .await
        .unwrap();
...
#[cfg(target_os = "linux")]
#[test]
fn catalog_rejects_an_artifact_that_exports_only_v1() {
    let (_fixture, artifact) = v1_only_fixture();
    let (_cache, catalog) = catalog();

    let error = catalog
        .load(artifact)
        .expect_err("ABI v2 must not probe or accept the removed v1 entry");
    assert!(
        error.to_string().contains("rsi_meta_plugin_entry_v2"),
        "{error}"
    );
}
...
fn catalog_preserves_a_plugin_entry_failure_status() {
    ...
    let (_fixture, artifact) =
        failed_entry_fixture(rsi_meta_plugin::STATUS_PANICKED, &exchange_log);
    let (_cache, catalog) = catalog();
...
async fn malformed_plugin_output_is_released_exactly_once_before_rejection() {
    ...
    assert_eq!(
        ...
        b"x",
        "the compatible v2 table's transferred output lease was not released exactly once"
    );
}
