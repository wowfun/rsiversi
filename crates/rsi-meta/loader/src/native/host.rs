...
impl HostLease {
    ...
    pub(super) fn table(&self) -> HostTable {
        HostTable {
            header: TableHeader::new(rsi_meta_plugin::ABI_MINOR, HostTable::STRUCT_SIZE),
...
impl HostState {
    ...
    fn dispatch(
        ...
        if !valid_output(output, output_capacity, expected) {
            return rsi_meta_plugin::STATUS_BUFFER_TOO_SMALL;
        }
    ...
    fn effect_defer(&self, input: *const c_void, input_size: u32) -> HostResult {
        ...
        if input.cleanup.kind != rsi_meta_plugin::CAP_KIND_CLEANUP
...
impl HostFailure {
    ...
    fn from_core(error: rsi_meta::MetaError) -> Self {
        let status = match error {
            ...
            rsi_meta::MetaError::Cancelled => rsi_meta_plugin::STATUS_CANCELLED,
...
fn read_frame<T: Copy>(input: *const c_void, input_size: u32) -> Result<T, HostFailure> {
    ...
    let header = unsafe { input.cast::<rsi_meta_plugin::FrameHeader>().read() };
...
fn output_size(opcode: u32) -> u32 {
    match opcode {
        ...
        HOST_PROVIDE => size_u32::<rsi_meta_plugin::CapOutput>(),
        _ => 0,
    }
}
