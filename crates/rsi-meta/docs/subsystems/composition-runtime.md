...
A workspace fixes the database, immutable cache, installed manifest, and installed lock. `open` and `install_offline` take the same database-derived exclusive lease. Two absent installed files mean an empty revision-zero graph; two present files are recovered and loaded; one without the other is `torn_installed_pair` unless the durable pair journal can finish or restore the transaction.
