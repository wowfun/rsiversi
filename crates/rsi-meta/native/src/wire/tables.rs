...
impl HostTable {
    ...
    #[allow(clippy::cast_possible_truncation)]
    pub const MIN_SIZE_V3_0: u32 =
        (core::mem::offset_of!(Self, exchange) + core::mem::size_of::<ExchangeFn>()) as u32;

    /// Checks a host table as consumed by an ABI v3.0 plugin.
    ...
    pub const fn is_compatible_for_plugin(&self, plugin_minor: u32) -> bool {
        self.header.has_common_shape()
            ...
            && self.header.struct_size >= Self::MIN_SIZE_V3_0
            && self.issuer != 0
            && !self.state.is_null()
...
impl PluginTable {
    ...
    const fn minimum_size_for_minor(minor: u32) -> Option<u32> {
        match minor {
            ABI_MINOR => Some(Self::MIN_SIZE_V3_0),
            _ => None,
        }
    }
