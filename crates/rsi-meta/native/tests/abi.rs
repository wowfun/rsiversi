...
#[test]
fn v3_operation_frames_are_exact_and_finalization_is_separate() {
...
#[test]
fn v3_layout_uses_fixed_width_ids_and_one_exchange_port() {
    assert_eq!(ABI_MAJOR, 3);
    assert_eq!(ABI_MINOR, 0);
    assert_eq!(MAX_DIAGNOSTIC_BYTES, 65_536);
    assert_eq!(PLUGIN_ENTRY_SYMBOL, b"rsi_meta_plugin_entry_v3\0");
...
RSI_META_ASSERT(RSI_META_ABI_MAJOR == 3u, "ABI major drift");
...
typedef uint32_t (*entry_fn)(const rsi_meta_host_table *, rsi_meta_plugin_table *,
                             uint32_t);
static entry_fn typed_entry = rsi_meta_plugin_entry_v3;
