...
impl NativeInstance for LifecycleInstance {
    fn activate(
        &mut self,
        _activation: &mut rsi_meta_native::Activation<'_>,
    ) -> Result<(), String> {
    ...
    fn serve(
        &mut self,
        _port: &[u8],
        _channel: &mut rsi_meta_native::ProviderChannel<'_>,
    ) -> Result<(), String> {
...
unsafe extern "C" fn unused_host_exchange(
    ...
    rsi_meta_native::STATUS_UNSUPPORTED
}
...
impl OpenPlugin {
    ...
    fn release_output(&self, release: rsi_meta_native::ReleaseId) -> u32 {
        self.status_only(
    ...
    fn retain(&self, capability: rsi_meta_native::CapId) -> u32 {
        let mut output = BasicOutput {
            prefix: rsi_meta_native::OutputPrefix::empty(
...
struct LifecycleSetup {
    ...
    prepared: CapId,
    identity_release: rsi_meta_native::ReleaseId,
}
...
fn prepare_lifecycle(plugin: &OpenPlugin) -> LifecycleSetup {
    ...
    let mut identity = BytesOutput {
        prefix: rsi_meta_native::OutputPrefix::empty(0),
...
fn create_lifecycle_instance(plugin: &OpenPlugin, prepared: CapId) -> CapId {
    ...
    assert_ne!(duplicate.prefix.release, rsi_meta_native::ReleaseId::EMPTY);
...
fn sdk_lifecycle_is_single_use_and_finalizes_only_after_all_ownership_returns() {
    ...
    assert_ne!(basic.prefix.release, rsi_meta_native::ReleaseId::EMPTY);
    ...
    assert_eq!(
        ...
        rsi_meta_native::ReleaseId::EMPTY
    );
}
...
fn exchange_accepts_declared_minor_suffix_and_rejects_undeclared_trailing_bytes() {
    ...
    assert_eq!(
        plugin.exchange(PLUGIN_IDENTITY, &input, &mut output),
        rsi_meta_native::STATUS_INVALID_ARGUMENT
    );
    assert_eq!(plugin.release_output(output.prefix.release), STATUS_OK);
...
fn plugin_capabilities_reject_foreign_stale_kind_and_rights_metadata() {
    ...
    for (mut capability, expected) in [
        (
            ...
            rsi_meta_native::STATUS_WRONG_CAPABILITY,
        ),
        (
            {
                let mut value = factory;
            ...
            rsi_meta_native::STATUS_STALE_CAPABILITY,
        ),
        (
            {
                let mut value = factory;
        ...
        assert_ne!(output.prefix.release, rsi_meta_native::ReleaseId::EMPTY);
        assert_eq!(plugin.release_output(output.prefix.release), STATUS_OK);
        capability = factory;
        let _ = capability;
    }
...
impl NativeInstance for InstanceDrop {
    fn activate(&mut self, _: &mut rsi_meta_native::Activation<'_>) -> Result<(), String> {
        Ok(())
    }
    fn serve(
        &mut self,
        _: &[u8],
        _: &mut rsi_meta_native::ProviderChannel<'_>,
    ) -> Result<(), String> {
        Ok(())
    }
}
...
fn panicking_factory_prepared_and_instance_drops_leave_the_table_finalizable() {
    ...
    assert_eq!(
        ...
        rsi_meta_native::STATUS_PANICKED
    );
    assert_eq!(plugin.release_output(basic.prefix.release), STATUS_OK);
...
fn malformed_or_panicking_entry_transfers_no_cleanup_authority() {
    ...
    assert_eq!(output.factory, rsi_meta_native::CapId::INVALID);
