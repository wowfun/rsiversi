...
`rsi-meta-native-loader` is the trusted native execution adapter for explicit
artifact paths. It resolves a path to an immutable `ResolvedFactory` and
...
wrong winner. One load may rekey at most eight times; a source that keeps
changing fails that load and releases its admission instead of retaining a
load permit for unbounded repeated hashing and staging. Durable and
live-staging budgets are independent. A cold artifact
...
The catalog resolves only `rsi_meta_plugin_entry_v3`. A v2-only artifact is
...
returning a `ResolvedFactory` whose native digest identity is owned by the
catalog resolver. The executable adapter remains private and cannot substitute
its own provenance. An explicit non-OK entry remains an entry failure;
...
The private native adapter implements core's per-attempt prepare and activate
...
then observes exactly one cached terminal outcome. The pre-v3 behavior of starting
...
registered instances. Factory destruction follows the last external resolved
factory, instance, or issued-capability owner; Runtime shutdown cannot claim
