        ...
        match result {
            ...
            Ok(None) => {
                ...
                state.terminal = Some(Terminal {
                    status: rsi_meta_native::STATUS_OK,
...
fn terminal_from_error(error: &MetaError) -> Terminal {
    let status = match error {
        MetaError::Cancelled => rsi_meta_native::STATUS_CANCELLED,
        MetaError::Timeout(_) => rsi_meta_native::STATUS_TERMINAL,
        MetaError::Busy { .. } => rsi_meta_native::STATUS_BUSY,
        MetaError::Reentrant { .. } => rsi_meta_native::STATUS_REENTRANT,
        MetaError::StaleCapability
        | MetaError::StaleService { .. }
        | MetaError::StaleContext { .. }
        | MetaError::FiberDisposed { .. } => rsi_meta_native::STATUS_STALE_CAPABILITY,
...
mod tests {
    ...
    use rsi_meta::{
        ...
        DeadlineLimits, InvocationContext, PluginFactory, PreparedActivation, Requirement, Result,
    ...
    async fn cancellation_is_live_while_receive_owns_the_caller_half() {
        ...
        let provider = runtime
            .root()
            .apply(
                crate::resolved_test_factory(Arc::new(ProviderFactory {
        ...
        let consumer = runtime
            .root()
            .apply(
                crate::resolved_test_factory(Arc::new(ConsumerFactory {
