...
output_frame!(
    rsi_meta_native::BasicOutput,
    rsi_meta_native::BytesOutput,
    rsi_meta_native::CapOutput,
    rsi_meta_native::BorrowedCapOutput,
    rsi_meta_native::MessageOutput,
    rsi_meta_native::BoolOutput,
    rsi_meta_native::PrepareOutput,
);
...
impl PluginTransport {
    pub(super) fn new(table: PluginTable) -> Self {
        debug_assert!(table.is_compatible_for_host(rsi_meta_native::ABI_MINOR));
        Self {
            table,
    ...
    pub(super) fn run_cleanup(self: &Arc<Self>, capability: CapId) -> Result<(), LoaderError> {
        self.basic_cap(
            rsi_meta_native::PLUGIN_RUN_CLEANUP,
            capability,
            "cleanup run",
        )
    }
...
pub(super) fn copy_bytes(
    bytes: rsi_meta_native::RawBytes,
    maximum: usize,
    operation: &'static str,
...
pub(super) fn status_result(
    ...
    if status == rsi_meta_native::STATUS_BUSY {
        return Err(LoaderError::Busy { operation });
    }
    if status == rsi_meta_native::STATUS_REENTRANT {
        return Err(LoaderError::Reentrant { operation });
    }
    let name = match status {
        rsi_meta_native::STATUS_INVALID_ARGUMENT => "invalid argument",
        rsi_meta_native::STATUS_FAILED => "failed",
        rsi_meta_native::STATUS_PANICKED => "panicked",
        rsi_meta_native::STATUS_PROTOCOL_ERROR => "protocol error",
        rsi_meta_native::STATUS_UNSUPPORTED => "unsupported",
        rsi_meta_native::STATUS_STALE_CAPABILITY => "stale capability",
        rsi_meta_native::STATUS_WRONG_CAPABILITY => "wrong capability",
        rsi_meta_native::STATUS_LIMIT_EXCEEDED => "limit exceeded",
        rsi_meta_native::STATUS_CANCELLED => "cancelled",
        rsi_meta_native::STATUS_TERMINAL => "terminal",
        rsi_meta_native::STATUS_BUFFER_TOO_SMALL => "buffer too small",
        _ => "unknown status",
    };
        ...
        fn prefix(self) -> OutputPrefix {
            ...
            match self {
                ...
                Self::UnownedDiagnostic => {
                    prefix.diagnostic = rsi_meta_native::RawBytes {
        ...
        match opcode {
            PLUGIN_FINALIZE => {
                if output_capacity != size_u32::<BasicOutput>() || output.is_null() {
                    return rsi_meta_native::STATUS_PROTOCOL_ERROR;
                }
            ...
            _ => rsi_meta_native::STATUS_PROTOCOL_ERROR,
        }
    }
