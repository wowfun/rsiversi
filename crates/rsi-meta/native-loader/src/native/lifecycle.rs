...
impl ModuleControl {
    ...
    /// v3 table, callback, pointer, and no-unwind contracts.
    pub(crate) unsafe fn load(
        ...
        // SAFETY: Symbol name and function type are the complete ABI v3 entry
        ...
        if !plugin.is_compatible_for_host(rsi_meta_native::ABI_MINOR) {
            host.retire_without_plugin();
            let error = LoaderError::IncompatibleAbi {
                host_major: rsi_meta_native::ABI_MAJOR,
                host_minor: rsi_meta_native::ABI_MINOR,
        ...
        let mut module = Self {
            ...
            queue,
            executor,
            identity: FactoryIdentity::native("entry-failed", digest.clone()),
            digest,
            callback_timeout,
...
impl NativeFactory {
    ...
    pub(crate) fn resolved(module: Arc<ModuleControl>) -> rsi_meta::ResolvedFactory {
        rsi_meta::ResolvedFactory::new(
            module.identity.clone(),
            rsi_meta::UpdateMode::Replayable,
            Arc::new(Self::from_module(module)),
        )
    }
}
...
fn read_identity(
    ...
    reply.release()?;
    Ok(FactoryIdentity::native(plugin, digest))
}
...
fn raw_bytes(bytes: &[u8]) -> rsi_meta_native::RawBytes {
    rsi_meta_native::RawBytes {
