...
const MAXIMUM_SOURCE_REKEYS: usize = 8;

#[derive(Debug)]
struct SourceRekeyBudget {
    remaining: usize,
}

impl SourceRekeyBudget {
    const fn new() -> Self {
        Self {
            remaining: MAXIMUM_SOURCE_REKEYS,
        }
    }

    fn consume(&mut self) -> Result<(), LoaderError> {
        if self.remaining == 0 {
            return Err(LoaderError::InvalidInput(format!(
                "native artifact changed more than {MAXIMUM_SOURCE_REKEYS} times while loading"
            )));
        }
        self.remaining -= 1;
        Ok(())
...
#[cfg(test)]
mod tests {
    use super::{MAXIMUM_SOURCE_REKEYS, SourceRekeyBudget};

    #[test]
    fn source_rekey_budget_accepts_exactly_its_documented_limit() {
        let mut budget = SourceRekeyBudget::new();
        for _ in 0..MAXIMUM_SOURCE_REKEYS {
            budget.consume().expect("rekey inside budget");
        }
        let error = budget.consume().expect_err("one rekey beyond the budget");
        assert!(error.to_string().contains("changed more than 8 times"));
    }
}
