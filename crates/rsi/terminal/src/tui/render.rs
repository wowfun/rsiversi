...
pub(super) struct View {
    ...
    pub(super) area: Rect,
    pub(super) hits: Vec<(u16, u16, u16, Anchor, Anchor)>,
}
...
impl View {
    ...
    pub(super) fn location(&self, state: &State, row: usize) -> Option<(usize, usize)> {
        if self.session.as_ref() != Some(state.header.session_id()) {
            return None;
        }
        state
            .transcript
            .locate(self.anchors.get(row).copied().flatten()?)
    }
    pub(super) fn belongs_to(&self, state: &State) -> bool {
        self.session.as_ref() == Some(state.header.session_id())
    }
    pub(super) fn hit(&self, x: u16, y: u16, end: bool) -> Option<Anchor> {
        self.hits
            .iter()
...
pub(super) fn draw(frame: &mut Frame<'_>, state: &State) -> View {
    let area = frame.area();
    ...
    frame.render_widget(
        Paragraph::new(super::super::terminal_text(&header)).style(
            Style::default()
                .fg(Color::Cyan)
    ...
    frame.render_widget(
        Paragraph::new(super::super::terminal_text(&location))
            .style(Style::default().fg(Color::DarkGray)),
        Rect::new(0, 1, area.width, 1),
    );
    let work = format!(
        " {}{}  ·  {} questions  ·  {} approvals  ·  {}",
        state.actual_model.as_deref().unwrap_or("No model call yet"),
        if state.busy {
            "  [request pending]"
        } else {
            ""
    ...
    frame.render_widget(
        Paragraph::new(super::super::terminal_text(&work))
            .style(Style::default().fg(Color::DarkGray)),
    ...
    editor_text.push('▏');
    editor_text.push_str(&super::super::terminal_text(&draft.text[draft.cursor..]));
    let cursor = super::super::terminal_text(&draft.text[..draft.cursor]).len();
    ...
    if let Some(answer) = &state.answer
        && let Some(question) = answer.request.questions.get(answer.answers.len())
    {
        let text = format!(
