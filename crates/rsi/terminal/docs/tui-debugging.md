...
[Terminal application contract](../../README.md#terminal-application),
[Session contract](../../session/README.md), and
[Host contract](../../service-host/README.md).

## Trace one operation

The launcher starts the selected ordinary Profile in
[`application.rs`](../../core/src/application.rs).
[`plugin.rs`](../src/plugin.rs) prepares the terminal application
and owns its entry and cleanup. [`tui/mod.rs`](../src/tui/mod.rs)
...
[`input.rs`](../src/tui/input.rs), then follow the
...
view. [`surfaces.rs`](../src/surfaces.rs) composes each
attachment's renderer sink and shared
[`SessionController`](../../client/src/controller.rs) in a Shell-owned
...
`selected` in
[`transcript.rs`](../../terminal-ui/src/transcript.rs).
...
[`render.rs`](../../terminal-ui/src/render.rs). Inspect
...
[`terminal.rs`](../src/tui/terminal.rs). Frames are
...
[`host_cli.rs`](../../core/src/host_cli.rs). A detached daemon redirects
stdout/stderr to its owner log, whose location is derived by
[`ServiceHostPaths::owner_log`](../../service-host/src/owner.rs).
...
to the [SQLite Store](../../../rsi-agent/store-sqlite/README.md); avoid editing
...
| The model picker is empty | Check configured Language models and committed registration gates in the [AI router](../../../rsi-ai/core/src/lib.rs). Compare deployment/model spelling with Settings. Enumeration does not query the provider. |
...
| The editor stalls behind a slow request | Inspect outstanding Work kinds, queue admission, and Submit/Cancel flags. Reproduce with pending futures in the [controller tests](../src/tui/tests.rs). |
...
| Copy is truncated, altered, or reported as unverified | Compare source selection boundaries and omitted ranges, then inspect [clipboard delivery](../src/tui/clipboard.rs). Native exact readback and an OSC52 request establish different levels of evidence. |
...
raw mode. The [PTY fixture](../../core/tests/service_host_cli/tui.rs)
